...
| `domains`                                  | the feature-scope Domains tab, which DevEye renders: custom domain names, the DNS records to publish, verification. `hint` and `service` are required; commits the server entry to `domains` (see [06-settings-panels](06-settings-panels.md#domains)) |
| `shareTier`                                | must be `'never'` for external modules until a third-party module has exercised the contract; anything else commits you to it (a server `items` entry, listings that read `ctx.sharing.scope()`), as DevEye's own modules do                           |
...
| `'members.read'`    | `ctx.deveye.members.list()`: `{ userId, name, isOwner, color }` per member (`color` is the account colour its live presence wears, `null` when never set)                                                                                                           |
